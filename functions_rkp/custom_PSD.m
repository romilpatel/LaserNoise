function [Pxx_custom, f] = custom_PSD(signal, Fs, NFFT, window, noverlap)
    ...
    % Power of the window (normalization factor)
    U = sum(window.^2);  % This accounts for the window's effect on signal power
    ...
    % Normalize PSD to account for window power (remove window effect)
    Pxx_custom = Pxx_custom / (U / win_length);
end
...
function compare_PSD(signal, Fs)
    ...
    title('Custom PSD Estimate (With Window Normalization)');
    xlabel('Frequency (Hz)');
